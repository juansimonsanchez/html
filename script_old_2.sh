#!/usr/bin/env bash
# ==============================================================================
# SCRIPT DE GENERACIÓN DE DOCUMENTACIÓN PROFESIONAL (HTML Autónomo + MkDocs Material)
# ==============================================================================
...
if ! command -v mkdocs &> /dev/null; then
    ...
    pip install --quiet mkdocs mkdocs-material pymdown-extensions 2>/dev/null || pip3 install --quiet mkdocs mkdocs-material pymdown-extensions 2>/dev/null
fi

# 2. Generar el script auxiliar module_builder.py
cat << 'EOF' > module_builder.py
...
MKDOCS_EXTRA_CSS = """
.md-typeset table.property-table {
    width: 100%;
...
.md-typeset table.property-table th {
    display: none !important;
}

.md-typeset table.property-table td {
...
.md-typeset table.property-table tr:nth-child(odd) td {
    background-color: #ffffff !important;
}

.md-typeset table.property-table tr:nth-child(even) td {
    background-color: #f8fafc !important;
}

.md-typeset table.property-table td:first-child {
    font-weight: 700;
    width: 28%;
    background-color: #f1f5f9 !important;
...
.md-typeset table.property-table td p {
...
CODE_KEYWORDS = [
    ...
    'P {', 'BODY {', 'BACKGROUND:', 'COLOR:', 'MARGIN:', 'PADDING:', 'DISPLAY:', 'FONT-FAMILY:',
...
def format_markdown_file(md_path):
    ...
    def convert_unicode_codeblock_md(match):
        ...
        if any(kw in code_upper for kw in ['SELECT ', 'INSERT ', 'UPDATE ', 'DELETE ', 'CREATE TABLE']):
            ...
            lang = "python"
        elif any(kw in code_upper for kw in ['SI ', 'SI-ENTONCES', 'SEGUN ', 'MIENTRAS ', 'REPETIR ', 'PARA ', 'FUNCION ', 'ALGORITMO', 'ESCRIBIR ']):
            lang = "pseudocode"
        return f"\n\n```{lang}\n{code}\n```\n\n"
...
def build_navbar_html(folder_name, current_page_title, prog_file, pres_file, teoria_files, tareas_files):
    nav = []
    ...
    if pres_file:
        nav.append(f'      <a href="{pres_file[1]}" class="nav-btn">Presentación</a>')

    if prog_file:
        nav.append(f'      <a href="{prog_file[1]}" class="nav-btn">Programación</a>')

    if teoria_files:
        nav.append('      <div class="dropdown">')
        nav.append('        <button class="dropbtn" type="button">Teoría ▾</button>')
        nav.append('        <div class="dropdown-content">')
        for title, fname in teoria_files:
            nav.append(f'          <a href="{fname}">{title}</a>')
        nav.append('        </div>')
        nav.append('      </div>')

    if tareas_files:
        nav.append('      <div class="dropdown">')
        nav.append('        <button class="dropbtn" type="button">Tareas y Prácticas ▾</button>')
        nav.append('        <div class="dropdown-content">')
...
def create_index_html(folder_path, folder_name, prog_file, pres_file, teoria_files, tareas_files):
    ...
    content = '\n'.join(body)
    navbar = build_navbar_html(folder_name, 'Inicio', prog_file, pres_file, teoria_files, tareas_files)
...
def create_mkdocs_config(folder_path, folder_name, prog_file, pres_file, teoria_files, tareas_files):
    ...
    lines.append('        name: Cambiar a modo oscuro')
    ...
    lines.append('        name: Cambiar a modo claro')
    lines.append('')
...
def inject_layout_to_html(filepath, folder_name, prog_file, pres_file, teoria_files, tareas_files):
    filename = os.path.basename(filepath)
    current_page_title = get_clean_page_title(filename, folder_name)
    navbar_html = build_navbar_html(folder_name, current_page_title, prog_file, pres_file, teoria_files, tareas_files)
    ...
    toc_match = re.search(r'(<nav[^>]*id=["\']TOC["\'][^>]*>.*?</nav>|<div[^>]*id=["\']TOC["\'][^>]*>.*?</div>)', html, re.DOTALL | re.IGNORECASE)
    if toc_match:
...
def process_module_folder(folder_path):
    ...
    with open(os.path.join(folder_path, "styles.css"), 'w', encoding='utf-8') as f:
        f.write(CSS_STYLES)
    ...
    for f in html_files:
        ...
        if 'presentacion' in f_lower or 'presentación' in f_lower or 'presenta' in f_lower or 'ppt' in f_lower:
            pres_file = (title, f)
    ...
    all_html = [f for f in os.listdir(folder_path) if f.endswith('.html')]
    for f in all_html:
        file_path = os.path.join(folder_path, f)
        if os.path.isfile(file_path):
            inject_layout_to_html(file_path, folder_name, prog_file, pres_file, teoria_files, tareas_files)
...
for mod_folder in "${module_folders[@]}"; do
    ...
    if [ -f "logo.png" ]; then
        cp logo.png "$mod_folder/" 2>/dev/null
        mkdir -p "$mod_folder/docs"
        cp logo.png "$mod_folder/docs/" 2>/dev/null
    fi
    ...
    (
        ...
        find . -maxdepth 1 -type f \( -iname "*.pptx" -o -iname "*.ppt" \) -print0 | while IFS= read -r -d '' pptx; do
            ...
            libreoffice --headless --convert-to pdf "$pptx" 2>/dev/null
...
MEOF
        ...
        find . -maxdepth 1 -type f -name "*.docx" -print0 | while IFS= read -r -d '' docx; do
            ...
            libreoffice --headless --convert-to pdf "$docx" 2>/dev/null
